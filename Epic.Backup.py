...
def MakeBackup_o(Folderpath):
    ...
    streamIo(Folderpath, GameBackup)#向临时文件夹复制游戏本体
    #打包游戏文件
    make_zip(tmp_path_n, GamePath+'.zip')#打包临时文件夹内容
    '''

    ##打包游戏文件
    make_zip(Folderpath, GamePath+'.zip')   #通过路径直接打包游戏本体
    add_file_into_package(tmp_path_n + F2, GamePath+'.zip')     #通过临时文件夹向压缩包插入配置文件
    add_file_into_package(tmp_path_n +'key.txt', GamePath+'.zip')     #通过临时文件夹向压缩包插入key文件
    ...
    Scrol.see(END)
    #提示
    tkinter.messagebox.showinfo('提示', '备份成功！备份包 '+ os.path.abspath('.') + '\\' + GamePath + '.zip')
